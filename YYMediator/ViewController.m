...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.row == 0) {
        UIViewController *viewController = [[YYMediator sharedInstance] YYMediator_viewControllerForDetail];
    ...
    if (indexPath.row == 2) {
        ...
        [[YYMediator sharedInstance] YYMediator_presentImage:[UIImage imageNamed:@"image"]];
    }
    
    if (indexPath.row == 3) {
        // 这种场景下，参数有问题，因此需要在流程中做好处理
        [[YYMediator sharedInstance] YYMediator_presentImage:nil];
    }
    
    if (indexPath.row == 4) {
        [[YYMediator sharedInstance] YYMediator_showAlertWithMessage:@"casa" cancelAction:nil confirmAction:^(NSDictionary *info) {
            // 做你想做的事
        }];
    }
